...
void step2(char *str, long long *iR, long long *iW, State *s)
{
    ...
    else if (prevState == State_Escaped && currState == State_Code)
    {
        str[*iW] = reallyShortCharStack;
        (*iW)++;
        reallyShortCharStack = -1;
    ...
    *s = currState;
    ...
    // output if requested and state is not comment
    if (b.output == 1 && !(currState == State_CComment || currState == State_CppComment))
    {
        str[*iW] = c;
        (*iW)++;
    }
